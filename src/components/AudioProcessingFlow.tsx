...
function Waveform({ bars, className = "", color }: { bars: number[]; className?: string; color: string }) {
  return (
...
/**
 * AI chip pipeline diagram — full-bleed dark section showing the six
 * processing stages between raw input and enhanced output, plus the
 * on-chip acceleration callout. Generic to the whole AI audio processor,
 * not specific to any one mic configuration.
 */
export default function AudioProcessingFlow() {
  return (
    <section
      id="audio-processor"
      // Full-bleed breakout: this section is dropped inside various
      // max-width containers, so it can't rely on the parent being full
      // width — it has to escape it explicitly to run edge-to-edge.
      className="relative left-1/2 right-1/2 -mx-[50vw] mt-28 w-screen px-4 py-28 text-white transition-colors duration-500 lg:px-6"
    ...
    >
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <h2 className="mt-3 text-headline font-semibold tracking-tight">Audio Processing Flow</h2>
        <p className="mt-4 max-w-2xl text-zinc-400">
          Our AI chip seamlessly processes audio from raw input to crystal-clear output.
        </p>
        ...
        <div className="mt-12 flex flex-col gap-2 xl:flex-row xl:items-stretch">
          ...
          <div className="flex flex-col items-center justify-center gap-2.5 rounded-[14px] border border-white/10 bg-white/[0.03] px-4 py-5 xl:w-28 xl:shrink-0">
            <Waveform bars={[55, 25, 70, 40, 85, 30, 60]} className="h-8" color={theme.textMuted} />
            ...
            <div key={stage.title} className="flex flex-1 flex-col gap-2 xl:flex-row xl:items-stretch">
              <div className="flex-1 rounded-[14px] border border-white/15 bg-white/5 p-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="font-geometric text-[10px]" style={{ color: ACTIVE_THEME.accent }}>
                    {String(i + 1).padStart(2, "0")}
                  </span>
                </div>
                <p className="mt-3 text-sm font-medium leading-tight">{stage.title}</p>
                <p className="mt-1.5 text-[11px] leading-relaxed text-zinc-400">{stage.desc}</p>
              </div>
              <div className="flex items-center justify-center">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4 shrink-0 rotate-90 text-zinc-600 xl:rotate-0" aria-hidden="true">
                  <path d="M5 12h14" />
          ...
          <div
            ...
            style={{ borderColor: ACTIVE_THEME.accentBorderMuted, backgroundColor: ACTIVE_THEME.accentBgMuted }}
          >
            <Waveform bars={[45, 60, 50, 65, 52, 62, 48]} className="h-8" color={ACTIVE_THEME.accent} />
            <p className="text-center text-[10px] font-semibold uppercase leading-tight tracking-[0.15em]" style={{ color: ACTIVE_THEME.accent }}>
        ...
        <div className="hidden xl:block">
          <div className="relative mt-3 h-7" style={{ marginInline: "10.5rem" }}>
            <div className="absolute inset-x-0 top-0 border-t border-dashed" style={{ borderColor: ACTIVE_THEME.accentBorderMuted }} />
            <div className="absolute left-0 top-0 h-3 border-l border-dashed" style={{ transform: "translateY(-100%)", borderColor: ACTIVE_THEME.accentBorderMuted }} />
            <div className="absolute right-0 top-0 h-3 border-r border-dashed" style={{ transform: "translateY(-100%)", borderColor: ACTIVE_THEME.accentBorderMuted }} />
            <div className="absolute left-1/2 top-0 h-full border-l border-dashed" style={{ borderColor: ACTIVE_THEME.accentBorderMuted }} />
          </div>
        </div>
        ...
        >
          ...
          <p className="text-sm text-zinc-400">Ultra-low latency · Low power · Real-time processing</p>
        </div>
      </div>
    </section>
  );
